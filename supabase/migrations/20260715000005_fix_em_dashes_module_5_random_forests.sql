...
-- AI/ML for Builders / Module 5: Random Forests
-- Replaces markdown glossary "**Term** — Definition" with "**Term**: Definition",
-- and general prose " — " with ", ". Titles use " - " (spaced hyphen).

update modules set description = $M$Learn how combining many decision trees produces more stable and accurate predictions, and how to apply random forests to real business problems.$M$ where id = '66b8d1a3-22aa-4ddf-ac27-0dfcb5e6777a';

update lessons set content = $D1601ef7f_3fc9_4f3e_b4dc_19199ccd1833$A random forest is a machine learning model made by combining many decision trees.
...
>> A single tree may be noisy. Many trees together are calmer. The group averages out individual mistakes.$D1601ef7f_3fc9_4f3e_b4dc_19199ccd1833$ where id = '1601ef7f-3fc9-4f3e-b4dc-19199ccd1833';

update lessons set content = $D7ddb9e45_8ab2_49c1_82c6_811831437fce$The word "random" in random forests comes from two types of deliberate randomness.
...
>> Random rows + random features = diverse trees = more stable predictions.$D7ddb9e45_8ab2_49c1_82c6_811831437fce$ where id = '7ddb9e45-8ab2-49c1-82c6-811831437fce';

update lessons set content = $D28f973f1_ede3_435c_b661_8da172c39ea5$To understand why random forests work, it helps to understand two failure modes: bias and variance.
...
>> Random forests reduce variance (instability) by averaging. They do not reduce bias (systematic error). Good input data and target definition are still the PM's responsibility.$D28f973f1_ede3_435c_b661_8da172c39ea5$ where id = '28f973f1-ede3-435c-b661-8da172c39ea5';

update lessons set content = $Dfe225b71_fbed_4855_833d_045066cb7f86$WHAT ARE THE INPUTS?
...
PM question:
  "What features are included, and would all of them be available at the time the prediction is needed?"$Dfe225b71_fbed_4855_833d_045066cb7f86$ where id = 'fe225b71-fbed-4855-833d-045066cb7f86';

update lessons set content = $Da17d4e6d_bfe2_43bf_9c75_290be3c5b003$WHAT IS FEATURE IMPORTANCE IN A RANDOM FOREST?
...
>> Feature importance is a starting point, not a conclusion. It shows what the model used, not what you should change.$Da17d4e6d_bfe2_43bf_9c75_290be3c5b003$ where id = 'a17d4e6d-bfe2-43bf-9c75-290be3c5b003';

update lessons set content = $D84ff764f_356e_40e6_81db_ba9f729c141f$Hyperparameters are settings chosen before training, they shape how the model is built. Understanding the key ones helps PMs ask the right questions during design and review.
...
>> Hyperparameters shape the model's behavior. As a PM, you do not need to tune them, but you should know what to ask.$D84ff764f_356e_40e6_81db_ba9f729c141f$ where id = '84ff764f-356e-40e6-81db-ba9f729c141f';

update lessons set content = $D2fd0388f_b144_4697_b203_6f6fef47f9d9$EXAMPLE 1: CUSTOMER CHURN PREDICTION
...
>> Both examples follow the same structure: features → forest probability → threshold → tiered action → business metric.$D2fd0388f_b144_4697_b203_6f6fef47f9d9$ where id = '2fd0388f-b144-4697-b203-6f6fef47f9d9';

update lessons set content = $D3423def3_b4b7_4e6f_8b7d_0fbae69a7a65$EXAMPLE 3: DEMAND FORECASTING
...
>> Demand and operations use regression forests; churn, fraud, and escalation use classification forests. The algorithm is the same, the target changes.$D3423def3_b4b7_4e6f_8b7d_0fbae69a7a65$ where id = '3423def3-b4b7-4e6f-8b7d-0fbae69a7a65';

update lessons set content = $Daab77416_aea7_4e55_b362_dd5878092cf3$CLASSIFICATION METRICS
...
>> The technical metric shows the model works. The business metric shows it matters.$Daab77416_aea7_4e55_b362_dd5878092cf3$ where id = 'aab77416-aea7-4e55-b362-dd5878092cf3';

update lessons set content = $D56636ac5_0956_4233_bdbb_5ae1db223e64$RANDOM FOREST VS. DECISION TREE
...
>> No model choice compensates for bad data. Choose the simplest model that meets the business requirement.$D56636ac5_0956_4233_bdbb_5ae1db223e64$ where id = '56636ac5-0956-4233-bdbb-5ae1db223e64';

update lessons set content = $D037a2f45_ab90_4b56_acc9_66d8d1400e46$WHAT CAN GO WRONG
...
The most important PM question:
  **Does the improved accuracy justify the added complexity compared with a simpler model?**$D037a2f45_ab90_4b56_acc9_66d8d1400e46$ where id = '037a2f45-ab90-4b56-acc9-66d8d1400e46';

update questions set choices = $D78e5f953_b8fa_43dd_9c0c_0d393671ec23Q$["Variance, instability caused by over-sensitivity to the training data","Bias, systematic errors caused by a model that is too simple","Data leakage, information from the future used in training","Class imbalance, too many negative examples in the training set"]$D78e5f953_b8fa_43dd_9c0c_0d393671ec23Q$::jsonb where id = '78e5f953-b8fa-43dd-9c0c-0d393671ec23';

update questions set choices = $D2889a2e4_5665_447c_954d_90d18498b9a6Q$["None, random forests work directly with features on different scales","Feature scaling so all variables have the same range","One-hot encoding of every categorical variable","Principal component analysis to reduce dimensionality"]$D2889a2e4_5665_447c_954d_90d18498b9a6Q$::jsonb where id = '2889a2e4-5665-447c-954d-90d18498b9a6';

update questions set choices = $Dbdf1d641_2199_4cbc_8ec9_d86bc780b16dQ$["Ask whether age is actionable, if not, identify which actionable features also matter","Remove age from the model since it cannot be changed","Conclude that age causes churn and design age-specific products","Retrain the model without age to see if performance drops"]$Dbdf1d641_2199_4cbc_8ec9_d86bc780b16dQ$::jsonb where id = 'bdf1d641-2199-4cbc-8ec9-d86bc780b16d';

update questions set choices = $D297fd58f_4b28_4966_8af6_4caab54cce75Q$["Segment-level error analysis for holiday periods, average error hides important failures","Increasing the number of trees to improve overall accuracy","Switching from regression to classification for seasonal periods","Removing holiday flag from the feature set since it introduces noise"]$D297fd58f_4b28_4966_8af6_4caab54cce75Q$::jsonb where id = '297fd58f-4b28-4966-8af6-4caab54cce75';

update questions set choices = $D2ee62a3b_5b67_48fc_a0d7_6caedf85833aQ$["Fraud is rare, so a model predicting not-fraud every time also achieves ~99% accuracy, precision, recall, and PR AUC matter more","99% accuracy is the theoretical maximum and confirms the model is perfect","Accuracy is only misleading for regression models, not classification","This means the model has 1% false positives, which is acceptable for most fraud systems"]$D2ee62a3b_5b67_48fc_a0d7_6caedf85833aQ$::jsonb where id = '2ee62a3b-5b67-48fc-a0d7-6caedf85833a';
